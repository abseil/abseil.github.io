...
Thinking back on my earliest experiences with google3 C++, I’m almost positive
...
That turns out to be critical, and easy to overlook, since the style guide
...
Borrowing an example from the [C++ Core
Guidelines](http://isocpp.github.io/CppCoreGuidelines/CppCoreGuidelines) , we
would not want to see something like:
...
(See go/totw/77 for more on move-semantics.)
...
the difference likely to be relevant. See go/totw/77 and go/totw/117.
