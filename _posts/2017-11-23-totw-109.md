...
is ignored, so both declarations are equivalent ([over.load][CppStandard]).
...
from [over.load][CppStandard] in the C++ standard (emphasis added):
...
[CppStandard]: http://eel.is/c++draft/
