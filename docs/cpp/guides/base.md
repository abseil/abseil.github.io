...
conjunction with standard library behavior (`std::vector` resizes much mor
...
(ASan) as an additional component, or which can be used standalone. Although
LSan and ASan are included or can be provided as additional components for most
compilers such as Clang, gcc and MSVC, the leak-checking API within Abseil is
only supported on Clang. Leak checking is enabled by default in all ASan builds.
...
```
# Enable just LSan
$ bazel build --copt=-fsanitizer=leak *target*

# Enable ASan, which also includes LSan.
$ bazel build --copt=-fsanitize=address --linkopt=-fsanitizer=address *target*
```
...
fundamental code that all other Abseil code depends on. As a result, the content
of base are kept to the minimum of what is absolutely necessary.
...
* `optimization.h` provides several platform-dependent macros for implementing
...
* `call_once.h` provides a primitive for ensuring a function is called exactly
  once, which is used during object initialization.
* `thread_annotations.h` provides a set of annotations to use within your code
